# -*- coding: utf-8 -*-
"""
Known mistakes: 10/19/24: Maga actually was said 3 times
"""
...
def percents_for_keyword(keywords, minCount, bAllowPartial, events_local):
    """
    
    keywords, minCount, bAllowPartial, events_local = keywords, row['Count'], row['AllowPartial'], events.copy()
    
    # For back testing
    keywords, minCount, bAllowPartial, events_local = keywords, row['Count'], row['AllowPartial'], events.query('Filename == %r'%(eventFilename)).copy()
    """
    ...
    for i,row in events_local.iterrows():
        ...
        for keyword in keywords:
            ...
            stringMatches.extend([[row['Filename'],contents[x[0]-15:x[1]+15],bAllowPartial] for x in matches])
    ...
    if events_local.loc[:,'Matches'].sum() > 0:
        percPartOfAnotherWord = events_local.loc[:,'PartOfAnotherWord'].sum() / events_local.loc[:,'Matches'].sum()
    else:
        percPartOfAnotherWord = 0.0
    
    return uw_perc_other, w_perc_other, uw_perc_rally, w_perc_rally, uw_perc_all, w_perc_all, percPartOfAnotherWord, stringMatches, events_local.copy()
...
# Backtest concluded contests
print('Backtesting contests')
for i,row in events.query('ContestFile != ""').iterrows():
    contest = pd.read_csv(row['ContestFile']).fillna('')
    contest.loc[:,'AllowPartial'] = contest.loc[:,'AllowPartial']==1
    eventFilename = row['Filename']
    error_checking = []
    events_dfs_list = []
    for i,row in contest.iterrows():
        keywords = [row['Main']]
        if row['AdditionalTerms'] != '':
            keywords.extend(row['AdditionalTerms'].split(','))
        # keywords = [x.upper() for x in keywords]
    
        temp = percents_for_keyword(keywords, row['Count'], row['AllowPartial'], events.query('Filename == %r'%(eventFilename)).copy())
        contest.loc[i,['uw_perc_other', 'w_perc_other', 'uw_perc_rally', 'w_perc_rally', 'uw_perc_all', 'w_perc_all', 'percPartOfAnotherWord']] = temp[:7]
        error_checking.append(temp[-2])
        events_dfs_list.append(temp[-1])
    # Don't match
    s_filter = ((contest.loc[:,'Results'] == 'Yes') & (contest.loc[:,'w_perc_rally']==0.0)) | ((contest.loc[:,'Results'] == 'No') & (contest.loc[:,'w_perc_rally']==1.0))
    print(eventFilename,'\n',contest.loc[s_filter,'Main'], '\nmatches:',len(s_filter)-np.sum(s_filter),'of',len(s_filter))


word = 'china'
text = processedText['20241018']
pattern = rf"\b{word}(s|'s)?\b"
re.search(pattern, text, re.IGNORECASE)
re.findall(pattern, text, re.IGNORECASE)
matches = [(match.start(), match.end()) for match in re.finditer(pattern, text, re.IGNORECASE)]
text_matches = [text[x[0]-15:x[1]+15] for x in matches]
